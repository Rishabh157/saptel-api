...
const { uploadFileS3 } = require("./S3UploadImage");
const { promises: Fs } = require("fs");
const { allMimetype } = require("./validation");
...
const uploadImageFunction = async (fileData, fieldName, routeData) => {
    try {
        ...
        let image_name = `${fieldName}_${fileData.fieldname}_${moment(new Date()).utcOffset("+05:30").format("YMMDDHHmmss")}.${path_array[path_array.length - 1]}`.toLowerCase();
                        ...
                        if (Array.isArray(req.files[field])) {
                            if (req.files[field].length) {
                                for (let each in req.files[field]) {
                                    ...
                                    if (!allMimetype.includes(fileData.mimetype)) {
                                        isError = true
                                        errMsg = `File extension for ${fieldName} is not valid. Only images are allowed.`
                                    }

                                    let imageUploadedData = await uploadImageFunction(fileData, imageTitle, folder_name)
                            ...
                            if (!Array.isArray(req.files[field]) && typeof req.files[field] === "object") {
                                ...
                                let imageUploadedData = await uploadImageFunction(fileData, imageTitle, routeData)
                                if (imageUploadedData.isUploaded) {
                                    isError = false
                                    errMsg = ""
                                    // imageObject.push([fileData.fieldname] = imageUploadedData.imageUrl)

...
exports.update = async (req, res) => {
  try {
    ...
    let dataUpdated = await websitePageService.getOneAndUpdate(
      {
        _id: idToBeSearch,
        isDeleted: false,
      },
...
exports.allFilterPagination = async (req, res) => {
  try {
    ...
    //-----------------------------------
    let dataFound = await websitePageService.aggregateQuery(
      finalAggregateQuery
    );
...
exports.deleteDocument = async (req, res) => {
  try {
    ...
    const deleteRefCheck = await deleteUser(collectionArrToMatch, 'websitePageId', _id)
...
exports.statusChange = async (req, res) => {
  try {
    let _id = req.params.id;
    let dataExist = await websitePageService.getOneByMultiField({ _id });
    if (!dataExist) {
      throw new ApiError(httpStatus.OK, "Data not found.");
    }
    let isActive = dataExist.isActive ? false : true;

    let statusChanged = await websitePageService.getOneAndUpdate(
      { _id },
      { isActive }
    );
    if (!statusChanged) {

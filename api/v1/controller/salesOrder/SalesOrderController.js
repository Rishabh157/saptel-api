...
exports.update = async (req, res) => {
  try {
    ...
    if (!datafound) {
      throw new ApiError(httpStatus.OK, `SalesOrder not found.`);
    }

    let dataUpdated = await salesOrderService.getOneAndUpdate(
      {
        _id: idToBeSearch,
        isDeleted: false,
      },
...
exports.allFilterPagination = async (req, res) => {
  try {
     ...
     */
    let booleanFields = [];
    let numberFileds = ["soNumber", "dealer", "wareHouse"];
...
exports.get = async (req, res) => {
  try {
    ...
    let dataExist = await salesOrderService.findAllWithQuery(matchQuery);
...
exports.getById = async (req, res) => {
  try {
    //if no default query then pass {}
    let idToBeSearch = req.params.id;
    let dataExist = await salesOrderService.getOneByMultiField({
      _id: idToBeSearch,
      isDeleted: false,
    });

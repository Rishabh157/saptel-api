...
exports.add = async (req, res) => {
  try {
     ...
     */
    let dataExist = await productGroupService.isExists([{ groupName }]);
    if (dataExist.exists && dataExist.existsSummary) {
      throw new ApiError(httpStatus.OK, dataExist.existsSummary);
    }
    //------------------create data-------------------
    let dataCreated = await productGroupService.createNewData({ ...req.body });
...
exports.update = async (req, res) => {
  try {
    ...
    if (!datafound) {
      throw new ApiError(httpStatus.OK, `ProductGroup not found.`);
    }

    let dataUpdated = await productGroupService.getOneAndUpdate(
      {
        _id: idToBeSearch,
        isDeleted: false,
      },
...
exports.allFilterPagination = async (req, res) => {
  try {
    ...
    //-----------------------------------
    let dataFound = await productGroupService.aggregateQuery(
      finalAggregateQuery
    );
    ...
    let result = await productGroupService.aggregateQuery(finalAggregateQuery);
...
exports.getForDealer = async (req, res) => {
  try {
    let companyId = req.userData.companyId;
...
exports.getById = async (req, res) => {
  try {
    ...
    let dataExist = await productGroupService.getOneByMultiField({
      _id: idToBeSearch,
      isDeleted: false,
    });
    let allowedFields = getAllowedField(fieldsToDisplay, dataExist);
...
exports.statusChange = async (req, res) => {
  try {
    let _id = req.params.id;
    let dataExist = await productGroupService.getOneByMultiField({ _id });
    if (!dataExist) {
      throw new ApiError(httpStatus.OK, "Data not found.");
    }
    let isActive = dataExist.isActive ? false : true;

    let statusChanged = await productGroupService.getOneAndUpdate(
      { _id },
      { isActive }
    );
    if (!statusChanged) {

...
// ----service---------
const prepaidOrderService = require("./PrepaidOrderService");
// ----service---------
...
const { searchKeys } = require("../../model/PrepaidOrderSchema");
...
exports.update = async (req, res) => {
  try {
    ...
    let dataExist = await prepaidOrderService.isExists([]);
    ...
    //------------------Find data-------------------
    let datafound = await prepaidOrderService.getOneByMultiField({
      _id: idToBeSearch,
    });
    if (!datafound) {
...
exports.get = async (req, res) => {
  try {
    ...
    let dataExist = await prepaidOrderService.aggregateQuery(additionalQuery);
    console.log("11", dataExist);
...
exports.allFilterPagination = async (req, res) => {
  try {
     ...
     */
    let booleanFields = [];
    let numberFileds = ["prepaidOrderNumber"];
    let objectIdFields = [
      "dispositionLevelTwoId",
      "dispositionLevelThreeId",
      "countryId",
    ...
    //-----------------------------------
    let dataFound = await prepaidOrderService.aggregateQuery(
      finalAggregateQuery
    );
    ...
    let result = await prepaidOrderService.aggregateQuery(finalAggregateQuery);
    if (result.length) {
...
exports.deleteDocument = async (req, res) => {
  try {
    ...
    if (!(await prepaidOrderService.getOneByMultiField({ _id }))) {
      throw new ApiError(httpStatus.OK, "Data not found.");
    }
    ...
    if (deleteRefCheck.status === true) {
      let deleted = await prepaidOrderService.getOneAndDelete({ _id });
      if (!deleted) {

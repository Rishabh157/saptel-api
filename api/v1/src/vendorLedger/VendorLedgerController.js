...
const ledgerService = require("./VendorLedgerService");
const { searchKeys } = require("./VendorLedgerSchema");
...
const { getVendorFromLedger, getBalance } = require("./VendorLedgerHelper");
const {
  getSearchQuery,
  checkInvalidParams,
  getRangeQuery,
...
exports.add = async (req, res) => {
  try {
    let { noteType, creditAmount, debitAmount, remark, companyId, vendorId } =
      req.body;
    /**
    ...
    const vendorExitsId = await getVendorFromLedger(vendorId);
    const balance = await getBalance(vendorExitsId, creditAmount, debitAmount);
...
exports.update = async (req, res) => {
  try {
    let { noteType, price, remark, companyId, vendorId } = req.body;
...
exports.allFilterPagination = async (req, res) => {
  try {
    ...
    let booleanFields = [];
    let numberFileds = [];
    let objectIdFields = ["companyId", "vendorId"];
    ...
    let additionalQuery = [
...

      {
        $addFields: {
          vendorLabel: {
            $arrayElemAt: ["$vendorData.companyName", 0],
          },
        },
      },
      {

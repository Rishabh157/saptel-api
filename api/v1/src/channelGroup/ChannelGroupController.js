...
const { searchKeys } = require("./ChannelGroupSchema");
...
exports.update = async (req, res) => {
  try {
    ...
    if (!datafound) {
      throw new ApiError(httpStatus.OK, `ChannelGroup not found.`);
    }

    let dataUpdated = await channelGroupService.getOneAndUpdate(
      {
        _id: idToBeSearch,
        isDeleted: false,
      },
...
exports.allFilterPagination = async (req, res) => {
  try {
    ...
    //-----------------------------------
    let dataFound = await channelGroupService.aggregateQuery(
      finalAggregateQuery
    );
    ...
    let userRoleData = await getUserRoleData(req, channelGroupService);
    let fieldsToDisplay = getFieldsToDisplay(
      moduleType.channelGroup,
      userRoleData,
      actionType.pagination
    );
    let result = await channelGroupService.aggregateQuery(finalAggregateQuery);
...
exports.statusChange = async (req, res) => {
  try {
    let _id = req.params.id;
    let dataExist = await channelGroupService.getOneByMultiField({ _id });
    if (!dataExist) {
      throw new ApiError(httpStatus.OK, "Data not found.");
    }
    let isActive = dataExist.isActive ? false : true;

    let statusChanged = await channelGroupService.getOneAndUpdate(
      { _id },
      { isActive }
    );
    if (!statusChanged) {

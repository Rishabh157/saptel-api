...
const { searchKeys } = require("./wtsMasterSchema");
...
const { getWTSCode } = require("./wtsMasterHelper");

//add start
exports.add = async (req, res) => {
  try {
    let { fromWarehouseId, companyId, toName, productSalesOrder, remark } =
      req.body;
    /**
     ...
     * check duplicate exist
     */
    let wtsNumber = await getWTSCode(companyId);
    let dataExist = await wtsMasterService.isExists([{ wtsNumber }]);
    ...
    //------------------create data-------------------
    let dataCreated = await wtsMasterService.createMany(output);
...
exports.update = async (req, res) => {
  try {
    const { wtsData } = req.body;

    const wtsIds = wtsData?.map((ele) => ele?.id);
    const allrtvOfThisNumber = await wtsMasterService?.findAllWithQuery({
      wtsNumber: wtsData[0]?.wtsNumber,
    });
    ...
    const updatePromises = wtsData?.map(async (ele) => {
      if (ele?.id !== "") {
        const dataUpdated = await wtsMasterService.getOneAndUpdate(
          {
...
exports.allFilterPagination = async (req, res) => {
  try {
    ...
    let booleanFields = [];
    let numberFileds = [];
    let objectIdFields = ["fromWarehouseId", "toName", "productGroupId"];
    ...
    //-----------------------------------
    let dataFound = await wtsMasterService.aggregateQuery(finalAggregateQuery);
    ...
    let result = await wtsMasterService.aggregateQuery(finalAggregateQuery);
    if (result.length) {
...
exports.allFilterGroupPagination = async (req, res) => {
  try {
    ...
    let numberFileds = [];
    let objectIdFields = ["fromWarehouseId", "companyId"];
    ...
    let additionalQuery = [
      ...
      {
        $unset: ["from_warehouse", "productSalesOrders"],
      },
    ];
    if (additionalQuery.length) {
      finalAggregateQuery.push(...additionalQuery);
    }
    let groupBywtsNumber = {
      $group: {
        _id: "$wtsNumber", // Group by the unique wtNumber field
    ...
    finalAggregateQuery.push(groupBywtsNumber);
...
exports.get = async (req, res) => {
  try {
    ...
    let dataExist = await wtsMasterService.findAllWithQuery(matchQuery);
...
//get by id
exports.getByWtsNumber = async (req, res) => {
  try {
    let wtsnumberToBeSearch = req.params.wtsnumber;
    let dataExist = await wtsMasterService.findAllWithQuery({
      wtsNumber: wtsnumberToBeSearch,
      isDeleted: false,
    });
...
exports.deleteDocument = async (req, res) => {
  try {
    let wtsnumber = req.params.wtsnumber;

    let deleted = await wtsMasterService.updateMany(
      {
        wtsNumber: wtsnumber,
        isDeleted: false,
      },
...
exports.statusChange = async (req, res) => {
  try {
    ...
    let dataExist = await wtsMasterService.getOneByMultiField({ _id });
    if (!dataExist) {
      throw new ApiError(httpStatus.OK, "Data not found.");
    }
    let isActive = dataExist.isActive ? false : true;

    let statusChanged = await wtsMasterService.getOneAndUpdate(
      { _id },
      { isActive }
    );
    if (!statusChanged) {
...
exports.updateLevel = async (req, res) => {
  try {
    ...
    //------------------Find data-------------------
    let datafound = await wtsMasterService.aggregateQuery([
      {
        $match: {
          wtsNumber: wtsNumberToBeSearch,
        },
    ...
    const dataUpdated = await wtsMasterService.updateMany(
      {
        wtsNumber: wtsNumberToBeSearch,
        isDeleted: false,
      },
...
// update wts status
exports.updatewtsStatus = async (req, res) => {
  try {
    let wtsnumber = req.params.wtsnumber;
    let status = req.params.status;
    let dataUpdated = await wtsMasterService.updateMany(
      {
        wtsNumber: wtsnumber,
        isDeleted: false,
      },

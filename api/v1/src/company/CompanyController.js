...
exports.allFilterPagination = async (req, res) => {
  try {
    ...
    //-----------------------------------
    let dataFound = await companyService.aggregateQuery(finalAggregateQuery);
    ...
    let userRoleData = await getUserRoleData(req, companyService);
    let fieldsToDisplay = getFieldsToDisplay(
      moduleType.company,
      userRoleData,
      actionType.pagination
    );
    let result = await companyService.aggregateQuery(finalAggregateQuery);
...
exports.get = async (req, res) => {
  try {
    ...
    let dataExist = await companyService.findAllWithQuery(matchQuery);
...
exports.getById = async (req, res) => {
  try {
    ...
    let dataExist = await companyService.getOneByMultiField({
      _id: idToBeSearch,
      isDeleted: false,
    });
    let allowedFields = getAllowedField(fieldsToDisplay, dataExist);
...
exports.statusChange = async (req, res) => {
  try {
    let _id = req.params.id;
    let dataExist = await companyService.getOneByMultiField({ _id });
    if (!dataExist) {
      throw new ApiError(httpStatus.OK, "Data not found.");
    }
    let isActive = dataExist.isActive ? false : true;

    let statusChanged = await companyService.getOneAndUpdate(
      { _id },
      { isActive }
    );
    if (!statusChanged) {
